...
from qgis.core import  QgsProject ,QgsVectorLayer,QgsDataSourceUri,QgsProperty,QgsField
    ...
    if layer.isSpatial():
        ...
        if "_pkt_"  in db_table_name.lower():
            if "label" not in layer.name().lower():
                ...
                if "richtungspfeil"  in layer.name().lower():
                    layer.addExpressionField(
                    """
                    map_get(layout, 'farbe')
                    """,
...
for layer_name in layer_list:
    ...
    for layer in layers:
        ...
        db_table_name = uri.table()
        if "_pkt_"  in db_table_name.lower() and "aw_pkt_topologie_v" != db_table_name.lower()  and "wa_pkt_beprobungspunkt" != db_table_name.lower():
            print(layer.name())
